function [KradNum,KradDen,Ainf_hat]=FDIRadMod(W,A,Ainf,B,FDIopt,Dof)
...
if FDIopt.AinfFlag==1,
    ...
    xlabel('Frequency [rad/s]')
     ...
     figure(1002) 
    subplot(221)
    plot(W,20*log10(abs(Kw)),'o-r',Wext2,20*log10(abs(Kw_hatFD)),'--b','LineWidth',2)
    xlim(Xpl)
    grid on
    legend('K(jw)',['K_{hat}(jw) order ',num2str(FDIopt.Ord)])
    ylabel('|K(jw)| [dB]') %N=Kg m/s^2 = (Kg/s) m/s  
    ...
    title(['Convolution Model DoF ',num2str(Dof(1)),num2str(Dof(2))])
    subplot(223)
    plot(W,(180/pi)*angle(Kw),'o-r',Wext2,(180/pi)*angle(Kw_hatFD),'--b','LineWidth',2)
    xlim(Xpl)
    grid on
    ylabel('Phase K(jw) [deg]') %N=Kg m/s^2 = (Kg/s) m/s  
    xlabel('Frequency [rad/s]')   
    ...
    figure(1002)
    subplot(224)
    plot(W,B,'o-r',Wext2,Brecfd,'-b','LineWidth',2)
    xlim(Xpl)
    grid on
    legend('B',['Best FD ident, order ', num2str(FDIopt.Ord)])
    ylabel('Damping') %N=Kg m/s^2 = (Kg/s) m/s  
    ...
    %
    subplot(222)
    plot(W,A,'o-r',Wext2,Arecfd,[Wext2(1) Wext2(length(Wext2))],[Ainf Ainf],'--b','LineWidth',2)
    xlim(Xpl)
    legend('A',['Aest FD indet, order ', num2str(FDIopt.Ord)],'Ainf')
    ylabel('Added Mass')
    xlabel('Frequency [rad/s]')
